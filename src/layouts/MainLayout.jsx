...
export const MainLayout = ({ children, onCommand, hideInput, onHistoryUp, onHistoryDown }) => {
  ...
  return (
    <div 
      className="relative h-[100dvh] w-screen bg-bg-secondary font-mono text-text-primary flex items-center justify-center p-[clamp(0rem,2vw,2rem)] transition-colors duration-300 overflow-hidden bg-vignette"
    ...
    >
      ...
      {/* Terminal Container */}
      <div className="relative w-full max-w-[clamp(100%,1200px,100%)] h-full sm:h-[clamp(500px,90dvh,1400px)] flex flex-col bg-bg-primary sm:rounded-lg overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-border-dark/30 terminal-window-glass transition-all duration-300 z-20">
        ...
        <div className="flex items-center justify-between px-4 py-2.5 bg-[#1a1a1a] border-b border-white/5 select-none z-[100]">
          <div className="flex space-x-2 w-[clamp(3rem,8vw,5rem)]">
            <div className="w-2.5 h-2.5 rounded-full bg-[#ff5f56]" />
            <div className="w-2.5 h-2.5 rounded-full bg-[#ffbd2e]" />
            <div className="w-2.5 h-2.5 rounded-full bg-[#27c93f]" />
          </div>
          <div className="text-fluid-xs text-text-secondary tracking-[0.2em] font-semibold opacity-60 uppercase truncate px-2">
             jayant@os ~ /portfolio
          </div>
          <div className="w-[clamp(3rem,8vw,5rem)]" /> {/* Spacer */}
        </div>
